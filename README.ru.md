# TrueNeverStory v0.15.0 – Платформа для создания интерактивных нарративных игр

**TrueNeverStory v0.15.0** – современная реализация платформы фэнтези-миров [BRING](https://github.com/Eva-E1/BRING), перенесённая с Python на высокопроизводительный гибридный стек:

- **TypeScript (Bun + Hono)** – Веб-сервер, API, WebSocket, маршрутизация, аутентификация, streaming, бизнес-логика
- **C FFI ядра (компилируются через Zig, с TS fallback)** – Вычислительные ядра для вероятностей и векторных операций (компилируются через Zig, с TypeScript fallback)
...
| **Планер сюжета** | Динамическое LLM-планирование арок, двухфазная генерация, адаптивное перепланирование |
...
│  └────────────────────────────────────────────────────┘  │
│  ┌────────────────────────────────────────────────────┐  │
│  │      C FFI Kernels (компилируются через Zig)        │  │
│  │  Probability Kernels │ Vector Operations           │  │
│  │  .so/.dylib/.dll → dlopen() или TypeScript fallback │  │
│  └────────────────────────────────────────────────────┘  │
└───────────────────────┬─────────────────────────────────┘
```
...
### Система (фоновая обработка)

| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| POST | `/api/system/pause` | Пауза цикла директора и очереди LLM |
| POST | `/api/system/resume` | Возобновление цикла директора и очереди LLM |
| GET | `/api/system/status` | Получить статус паузы/работы |
...
### C FFI ядра и кросс-компиляция (v0.14.1)

Перенесение вычислительных ядер Mojo в чистый C с кросс-компиляцией Zig для 10 платформ:

| Возможность | Описание |
|-------------|----------|
| **C FFI ядра** | 5 вычислительных ядер перенесены из Mojo в чистый C (probability, vector, vector_full, batch_ops, graph_ops) |
| **Кросс-компиляция Zig** | Единый скрипт сборки для Linux, macOS, Windows, ARM, RISC-V |
| **10 платформ** | aarch64/x86_64 Linux (glibc+musl), macOS, Windows, ARMv7, RISC-V |
| **Пакеты для распространения** | Каждый архив релиза содержит бинарник + FFI .so/.dll + public/ + .env |
| **Пауза/возобновление** | Цикл директора и очередь LLM приостанавливаются, когда пользователь покидает чат |

**Новые файлы:**
- `mojo/kernels/c/probability_ffi.c` — Ядра вероятностей (шанс успеха, бросок, пакетные)
- `mojo/kernels/c/vector_ffi.c` — 4-мерные векторные операции (косинус, L2, скалярное произведение)
- `mojo/kernels/c/vector_full.c` — Полноценные векторные операции (768-мерные)
- `mojo/kernels/c/batch_ops.c` — Пакетные операции NPC (угасание по возрасту, пороки, налоги, лояльность)
- `mojo/kernels/c/graph_ops.c` — Обход графов, RRF слияние, репутация
- `mojo/kernels/build.sh` — Кросс-компиляция через Zig
- `src/routes/system.ts` — API эндпоинты паузы/возобновления

**Изменённые файлы:**
- `src/services/director-loop.ts` — Добавлены методы `pause()`/`resume()`
- `src/lib/llm-queue.ts` — Добавлены методы `pause()`/`resume()`
- `src/services/narrative-service.ts` — Делегирование `pause()`/`resume()`
- `public/index.html` — Автопауза при уходе со страницы, автовозобновление при загрузке

### Расширение Mojo ядер (v0.12.0)

Масштабное повышение производительности вычислительных ядер Mojo для векторного поиска, пакетных операций NPC и обхода графов:

| Возможность | Описание |
|-------------|----------|
| **Ядро вероятностей** | Шанс успеха, результат броска, модификатор + пакетные вероятности через Mojo FFI |
| **Векторное ядро** | Косинусное сходство, L2 расстояние, скалярное произведение для 4-мерных векторов через Mojo FFI |
| **Полноценные векторы** | 768-мерные BGE-M3 эмбеддинги — пакетное косинусное сходство через Mojo FFI |
| **Пакетные операции NPC** | Угасание по возрасту, пороки, налоги, подсчёт богатства, проверки лояльности через Mojo FFI |
| **Графовые операции** | RRF слияние, сила отношений, вычисление репутации через Mojo FFI |
| **Ускорение SQLite** | searchDense/searchMemoriesDense используют пакетное косинусное сходство |

**Новые файлы:**
- `mojo/kernels/vector_full.mojo` — Полноценные векторные операции (косинус, L2, скалярное произведение, пакетные)
- `mojo/kernels/batch_ops.mojo` — Пакетные операции статистик NPC (угасание по возрасту, пороки, налоги, лояльность)
- `mojo/kernels/graph_ops.mojo` — Обход графов и RRF слияние
- `src/lib/mojo-ffi.test.ts` — 19 тестов для всех FFI привязок

**Изменённые файлы:**
- `mojo/kernels/probability_ffi.mojo` — Добавлены batch_success_chance и batch_roll
- `src/lib/mojo-ffi.ts` — 5 привязок ядер с TypeScript fallback'ами
- `src/lib/vector-ops.ts` — Использует ускоренный Mojo cosineSimilarity
- `src/lib/sqlite-store.ts` — searchDense/searchMemoriesDense используют batchCosineSimilarity
- `build.sh` — Компилирует все 5 ядер (probability, vector_4dim, vector_full, batch_ops, graph_ops)
...
### Социальные и политические системы (v0.11.0)

Новые системы социальной симуляции, диалогов, квестов и инвентаря:

| Возможность | Описание |
|-------------|----------|
| **Феодальная иерархия в SocialGraph** | Присяга, лорды/вассалы, цепочка командования, лояльность, восстание |
| **Система фракций** | 6 типов (военная/экономическая/религиозная/криминальная/дворянская/нейтральная), лидеры, влияние, казна |
| **Политические альянсы** | 5 типов (военный/торговый/оборонительный/ненападения/вассальный), предательство, репутация |
| **Диалоги NPC** | Управление сессиями, 11 категорий тем, контекстные приветствия/прощания, mood-driven |
| **Система квестов** | 5 типов квестов, 7 типов целей, награды, предусловия, цепочки, лимиты времени |
| **Планер сюжета** | Динамическое LLM-планирование арок, двухфазная генерация, адаптивное перепланирование |
| **Система инвентаря** | Редкость предметов (5 уровней), слоты экипировки, вес/ёмкость, золото, торговля |

**Новые файлы:**
- `src/services/social-graph.ts` — Отношения, фракции, альянсы, феодальная иерархия
- `src/services/dialogue-manager.ts` — Сессии диалогов NPC и темы
- `src/services/quest-system.ts` — Жизненный цикл квестов, цели, награды, цепочки
- `src/services/inventory-manager.ts` — Управление предметами, экипировка, торговля

### Система экономики NPC (v0.11.0)
...
### Хранение в SQLite для промптов и переводов (v0.11.0)
...
### Новые специализированные агенты (v0.11.0)
...
### Система RAG для всех агентов (v0.11.0)
