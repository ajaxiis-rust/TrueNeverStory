...
> Updated for v0.33.4 — RoleplayEngine refactored with SessionState, CommandHandler, PipelineRunner, Prose strategies.
...
TrueNeverStory follows a **layered onion (hexagonal) architecture** at its core, wrapped with an **event-driven orchestration layer** for asynchronous narrative processing. As of v0.33.4, the engine uses a **State-First pipeline** where deterministic simulation happens before prose generation.
...
### Gutenberg Processing Pipeline (v0.33.4)
...
### Dual Model Architecture (v0.33.4)
...
| `RoleplayEngine` | `src/services/roleplay-engine.ts` | Main entry point: orchestrates PipelineRunner → CommandHandler → Prose generators. SessionState extracted to `roleplay/session-state.ts`, handlers in `roleplay/handlers/` |
