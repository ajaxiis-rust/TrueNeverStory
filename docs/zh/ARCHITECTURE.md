...
> 已更新至 v0.32.5 — `RoleplayEngine` 重构为 `SessionState`、`CommandHandler`、`PipelineRunner`、散文策略。
...
TrueNeverStory 在其核心采用**分层洋葱（六边形）架构**，外层包裹一个**事件驱动编排层**用于异步叙事处理。自 v0.32.5 起，引擎采用**状态优先（State-First）管道**，确定性的模拟在散文生成之前执行。
