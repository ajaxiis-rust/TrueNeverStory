...
| **crafter** | Recipe suggestion prompt with dynamic scenario generation. Too complex for template vars. Now includes item uniqueness evaluation via historian/researcher. |
...
## Item Boost System (v0.10.2)

When a player crafts an item, the system evaluates its uniqueness and potential benefits:

### Evaluation Flow

1. **Player crafts item** → CrafterAgent.craftWithEvaluation()
2. **Historian checks** → Has this item existed before in this world?
3. **If unique** → Researcher evaluates benefits and target group
4. **Result** → Item gets permanent stat boost (1-10%)
...
interface ItemBoost {
  ...
  targetGroup?: string; // "беззубые старики", "учёные"
  reason: string;
}
```
...
```
Игрок создаёт мясорубку:
├── Historian: "В этом мире не существовало" → isUnique: true
├── Researcher: "Полезна для беззубых стариков" → +5% health
├── Предмет: [УНИКАЛЬНЫЙ] → +5% health (для беззубых)
└── Игрок может принять или отказаться
```

### Limit

- Максимум 10 уникальных предметов на NPC
- Бусты постоянные
