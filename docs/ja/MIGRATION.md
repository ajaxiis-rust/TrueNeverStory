...
## v0.32.5 移行: 文学コンパイラ & 経済モデル

v0.32.5 リリースでは文学コンパイラと経済モデルが追加される。移行は不要 — これらは既存の State-First パイプラインを拡張する追加機能である。

## v0.32.5 移行: State-First パイプライン

### 変更点

v0.32.5 リリースでは state-first パイプラインアーキテクチャが導入される。2 つのエージェントシステムが共存するようになった:
...
v0.32.5 では外部知識アクセスのために Model Context Protocol (MCP) ツールが導入される:
