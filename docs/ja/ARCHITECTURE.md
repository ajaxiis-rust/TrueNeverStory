...
> v0.33.4 向けに更新 — RoleplayEngine を SessionState、CommandHandler、PipelineRunner、Prose 戦略にリファクタリング。
...
TrueNeverStory は、その中核に**レイヤード・オニオン（ヘキサゴナル）アーキテクチャ**を採用し、非同期ナラティブ処理のための**イベント駆動オーケストレーションレイヤー**でラップしている。v0.33.4 以降、エンジンは**State-First パイプライン**を使用し、散文生成の前に決定的なシミュレーションが実行される。
