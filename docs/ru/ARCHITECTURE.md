...
> Обновлено для v0.33.0 — рефакторинг RoleplayEngine с SessionState, CommandHandler, PipelineRunner и стратегиями прозы.
...
TrueNeverStory в своём ядре следует **слоёной архитектуре «луковицы» (гексагональной архитектуре)**, обёрнутой **слоем событийной оркестрации** для асинхронной нарративной обработки. Начиная с v0.33.0 движок использует **конвейер State-First**, в котором детерминированная симуляция выполняется до генерации прозы.
...
### Конвейер обработки Gutenberg (v0.33.0)
...
### Архитектура с двумя моделями (v0.33.0)
...
### BC12: Литературный компилятор v2 (v0.33.0)
