...
- **Фаза A (V1, правил-based)** — очистка текста, 4-проходная литературная компиляция, без LLM
- **Фаза B (V2, LLM)** — AnalyzePass + NarrativeExtractor для шаблонов сцен и стилевых паттернов
- **Расширение корпуса** — загрузка дополнительных книг из Gutendex API (59 → 250+)
- **MCP-эндпоинт** — запуск пайплайна из браузера на `/mcp/gutenberg/process`

### Анализ и извлечение
- **AnalyzePass** — унифицированный анализ чанков с классификацией сцен (8 типов), сенсорными тегами (10 категорий), временными маркерами
- **NarrativeExtractor** — извлечение сюжетных арок, арок персонажей, тематических мотивов, морального вектора через LLM
- **Общая очистка текста** — три разных функции очистки объединены в одну `cleanGutenbergText()`

### Стилевые профили игроков
- **PlayerProfileStore** — 14 отслеживаемых метрик на игрока (длина предложений, сенсорное смещение, регистр, доля диалогов, литературная сложность и др.)
- Кросс-агентные профили, разделяемые между Стилистом и LiteraryV2Generator
- Обновления с учётом достоверности (confidence-weighted)

### Расширения Literary Compiler
- **DramaturgicPass prose mode** — генерация прозаических шаблонов из анализа чанков
...
### v0.28.0 — Literary Compiler и экономические модели
...
### v0.28.0 — Архитектура State-First
