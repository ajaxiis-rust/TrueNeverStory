...
./startgame.sh          # par défaut : --remote

# Windows PowerShell
tar xzf tns-windows-x64.zip
cd tns-windows-x64
.\startgame.ps1         # par défaut : --remote
```

Options de lancement :
- `--local` — se connecter à une instance Ollama locale
- `--remote` — utiliser l'API LLM distante (par défaut)
...
- Les 5 thèmes (Sombre, Clair, Terminal, Cyberpunk, Personnalisé) fonctionnent maintenant correctement via les boutons sélecteurs

### v0.20.4 — Fix graphe monde + modal statistiques + injection langue + thèmes
