...
| **ストーリープランナー** | LLM駆動の動的アーク計画、2フェーズ生成、適応的再計画 |
...
| 機能 | 説明 |
|------|------|
| **封建的階層** | 宣誓、領主/家臣、指揮系統、忠誠、反乱 |
| **派閥システム** | 6タイプ（軍事/経済/宗教/犯罪/貴族/中立）、リーダー、影響力 |
| **政治同盟** | 5タイプ（軍事/貿易/防衛/不可侵/従属）、裏切り、評判 |
| **NPC対話** | セッション管理、11のトピックカテゴリ、コンテキスト挨拶 |
| **クエストシステム** | 5タイプ、7タイプの目標、報酬、前提条件、チェーン |
| **ストーリープランナー** | LLM駆動の動的アーク計画、2フェーズ生成、適応的再計画 |
| **インベントリシステム** | レアリティ（5段階）、装備スロット、重量/容量、取引 |

**新規ファイル:** `social-graph.ts`, `dialogue-manager.ts`, `quest-system.ts`, `inventory-manager.ts`
